"""FAST training script for VizDoom DEADLY_CORRIDOR (NO RENDERING).
...
Using optimized hyperparameters from defend_the_center experience.
"""
import os
import sys
...
N_ENVS = 16  # Balanced for memory
LEARNING_RATE = 3e-4
BATCH_SIZE = 512
N_STEPS = 256
N_EPOCHS = 4  # Fast updates
GAMMA = 0.99
GAE_LAMBDA = 0.95
CLIP_RANGE = 0.2
...
def make_env(rank: int, seed: int = 0) -> Callable:
    ...
    def _init():
        env = DoomEnvWrapper(
            ...
            frame_stack=4,
            render_mode=None  # NO RENDERING - fast training
        )
...
def main():
    """Main training loop."""
    print("=" * 60)
    print(f"FAST TRAINING - {SCENARIO.upper().replace('_', ' ')}")
    print("=" * 60)
    ...
    print("\nExpected: This will take longer to learn than defend_the_center")
