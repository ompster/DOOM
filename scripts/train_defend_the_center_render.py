"""Training script for VizDoom DEFEND_THE_CENTER with rendering.

This version is optimized for stability and visibility:
- 8 parallel environments (faster startup, less memory)
...
N_ENVS = 8  # Fewer envs = faster startup, easier to debug
...
def make_env(rank: int, seed: int = 0) -> Callable:
    ...
    def _init():
        ...
        print(f"[Env {rank}] Initializing{'(WITH RENDERING)' if rank == 0 else ''}...")
...
def main():
    ...
    print(f"  Parallel environments: {N_ENVS} (fewer = faster startup)")
    ...
    print("VizDoom window should be visible now (Environment #0)")
    print("=" * 60)
